...
describeWithSqlite("storage schema migrations", () => {
  it("creates a fresh v2 schema with General and prompt version 1 defaults", async () => {
    ...
    expect(database.prepare("SELECT MAX(version) AS version FROM schema_migrations").get().version).toBe(2);
    database.close();
  });

  it("migrates v1 rows to the explicit legacy prompt version zero exactly once", async () => {
    const database = await openDatabase();
    database.exec(`
    ...
    expect(database.prepare("SELECT COUNT(*) AS count FROM schema_migrations WHERE version = 2").get().count).toBe(1);
    database.close();
  });
});
