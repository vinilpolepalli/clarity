...
function ModelsPage({ preferences, model, update, setModel }: PageProps) {
  ...
  async function removeCustomModel(id: string) {
    ...
    if (preferences.model === id) patch.model = model.providerCatalog.curated[0]?.id ?? model.defaultProviderModels[provider];
    await update(patch);
  }
...
function AudioPage({ preferences, model, update }: PageProps) {
  return <><PageTitle eyebrow="Capture" title="Audio" description="Choose explicit sources and see permission health before a meeting." />
    <Section title="Sources">
      ...
      <SettingRow icon={<AudioLines size={16} />} title="Default live-notes source" description="Each session can override this before listening begins."><Select label="Default live-notes source" value={preferences.meetingAudioSource} onChange={(value) => update({ meetingAudioSource: value as Preferences["meetingAudioSource"] })} options={[{ value: "microphone", label: "Microphone" }, { value: "system", label: "System audio" }, { value: "both", label: "Both" }]} /></SettingRow>
    </Section>
    <Section title="Local transcription" description="Live notes use a local Whisper CLI. Audio batches are deleted immediately after transcription.">
      <SettingRow icon={<FileText size={16} />} title="Whisper executable" description="The local command Clarity invokes for each finalized speech batch."><input className="settings-input small-input" aria-label="Whisper executable" value={preferences.whisperExecutable} onChange={(event) => update({ whisperExecutable: event.target.value })} /></SettingRow>
      <SettingRow icon={<FileText size={16} />} title="Whisper model" description="Absolute path to a local Whisper model file. Live notes stay unavailable until this is set."><input className="settings-input small-input" aria-label="Whisper model path" placeholder="/path/to/model.bin" value={preferences.whisperModelPath} onChange={(event) => update({ whisperModelPath: event.target.value })} /></SettingRow>
    </Section>
